    ...
    <div className="space-y-6 font-jakarta text-slate-900 select-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/80 pb-5">
        ...
        <div className="flex items-center gap-2 font-mono text-xs">
          <span className="text-emerald-800 font-bold bg-emerald-100 px-3.5 py-1.5 rounded-full border border-emerald-300">
      ...
      <div className="bg-slate-900 text-white rounded-[32px] p-6 sm:p-8 shadow-2xl space-y-6 border border-slate-800">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <span className="font-outfit text-sm font-bold text-slate-300 uppercase tracking-wider flex items-center gap-2">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 p-6 rounded-2xl bg-slate-950 border border-slate-800">
          <div>
            <div className="text-xs font-mono text-slate-400 uppercase tracking-wider">CURRENT OVERALL STATUS</div>
            <div className={`text-5xl font-extrabold font-outfit tracking-tight mt-1 ${
          ...
          <div className="w-full md:w-1/2 space-y-3 font-mono text-xs">
            <div className="flex justify-between text-slate-400 text-[11px] font-bold">
              <span className={overall.label === 'GOOD' ? 'text-emerald-400 text-xs' : ''}>GOOD</span>
              <span className={overall.label === 'MODERATE' ? 'text-amber-400 text-xs' : ''}>MODERATE</span>
              <span className={overall.label === 'POOR' ? 'text-rose-400 text-xs' : ''}>POOR</span>
              <span className={overall.label === 'CRITICAL' ? 'text-rose-500 text-xs' : ''}>CRITICAL</span>
            </div>
        ...
        <div className="pt-4 border-t border-slate-800">
          <h3 className="font-outfit text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {gases.map((g) => {
              ...
              return (
                <div key={g.formula} className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-2 text-center font-mono shadow-sm">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-bold font-outfit text-white text-base">{g.formula}</span>
                    <span className={`w-2.5 h-2.5 rounded-full ${stat.severity === 'normal' ? 'bg-emerald-400 animate-pulse' : stat.severity === 'warning' ? 'bg-amber-400' : 'bg-rose-500'}`} />
                  </div>
